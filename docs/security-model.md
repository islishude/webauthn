...
The current default remains conservative. Without a caller-supplied `attestation.TrustPolicy`, registration accepts only `none` attestation when `AllowNone` is true and rejects all non-`none` attestations after format verification. Optional `packed`, `fido-u2f`, `tpm`, `android-key`, and `android-safetynet` verification can prove statement validity, but x5c trust-chain acceptance is still a relying-party decision.

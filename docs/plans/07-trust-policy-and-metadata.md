...
2026-06-02: Plan 15 removed the separate registration attestation policy
surface. Registration acceptance now goes only through
`AttestationTrustPolicy`; nil trust policy rejects all attestations after format
verification, and consumer passkey `none` acceptance is represented by
`attestation.AcceptNone()`.
